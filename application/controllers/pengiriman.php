		...
		if ($post) {
			...
			if (empty($error)) {
				...
				if ($status === false || $status === null) {
					$status = 1; // Set default status to 1 if not valid
				}
				...
				// $total_rows = $this->pengiriman_model->count();
				// var_dump($total_rows);
				// die();
				...
				foreach ($datailkode as $key => $val) {
					...
					// $qty = $this->pengiriman_model->get_qty_by_id_pengiriman($data['id_pengiriman']);
					// if ($qty !== null) {
					// 	var_dump($qty);
					// 	die();
					// } else {
					...
					// 	die();
					// }
					// Simpan detail pengiriman dengan status
					$this->pengiriman_model->save_detail($detail, $status);
				}
	...
	public function delete($id = "")
{
    ...
    if (!empty($id)) {
        // Cek apakah ID pengiriman terdaftar
        ...
        if (empty($cek)) {
            ...
            $this->db->delete('detail_pengiriman');

            // Hapus data dari tabel barang_pengiriman
